...
spt_find_page (struct supplemental_page_table *spt UNUSED, void *va UNUSED) {
	...
	elem = hash_find(&spt->spt_hash, elem);
	free(page);
...
vm_stack_growth (void *addr UNUSED) {
	// vm_alloc_page(VM_UNINIT, addr, 1)
	// stack_bottom = thread_current()->rsp_stack - PGSIZE;
}
...
vm_try_handle_fault (struct intr_frame *f UNUSED, void *addr UNUSED,
	...
	/* TODO: Your code goes here */
	// Sangju did. 6.17. Sunghwan did. 6.19. 
	...
	if(not_present){
		if(!vm_claim_page(addr)){
			if (rsp_stack - 8 <= addr && addr <=USER_STACK){ // YeonJu: 이 부분 이해 안 감.
...
supplemental_page_table_kill (struct supplemental_page_table *spt UNUSED) {
	...
	while(hash_next(&i)){
		...
		if (page->operations->type == VM_FILE) //munmap(page->va);
			hash_destroy(&spt->spt_hash, remove_spt);	// Yeonju 6.17
	} 
}
